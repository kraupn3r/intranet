...
class TestDayListView(TestCase):
    ...
    def test_view_redirect_if_not_logged_in(self):
        ...
        self.assertRedirects(
            response, '/accounts/login/?next=/calendar/2020/6/')
    ...
    def test_view_proper_queryset_length(self):
        ...
        for each in range(1, 12):
            response = self.client.get(reverse('calendary:calendary', kwargs={
                                       'year': '2020', 'month': each}))
...
class TestPOST_Devent_View(TestCase):
    ...
    def test_view_redirect_if_not_logged_in(self):
        response = self.client.get(
            reverse('calendary:post_devent', kwargs={'pk': self.test_day.pk}))
        self.assertEquals(response.status_code, 302)
        self.assertRedirects(
            response, '/accounts/login/?next=/calendar/153/addevent/')

    def test_view_redirect_if_logged_in_no_permission(self):
        login = self.client.login(
            username='testuser1', password='1X<ISRUkw+tuK')
        response = self.client.get(
            reverse('calendary:post_devent', kwargs={'pk': self.test_devent.pk}))
